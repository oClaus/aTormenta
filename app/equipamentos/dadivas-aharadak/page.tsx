...
import { aharadak } from "@/data/dadivas-aharadak";
...
// Componente para a Tabela Filtrável de Equipamentos (Animais)
      ...
      <div className="overflow-x-auto rounded-xl border-2 border-amber-900/20 shadow-sm w-full bg-[rgb(var(--bg-card-rgb))]">
               ...
               return (
                <tr key={item.id} className={`${rowClass} hover:bg-[rgb(var(--bg-card-rgb))]/60 transition-colors group`}>
                  <td className="px-4 py-4 border-r-2 border-amber-900/10 align-top">
                    ...
                    <div className="text-sm text-amber-950/85 break-words font-serif whitespace-pre-line font-medium mt-1 leading-relaxed">{item.description}</div>
        ...
        {filteredGear.length === 0 && (
          <div className="text-center py-12 text-amber-950/70 bg-[rgb(var(--bg-inset-rgb))] italic text-lg border-t-2 border-amber-900/20 flex flex-col items-center gap-3">
            ...
            Nenhuma dádiva encontrada com os filtros aplicados.
          </div>
        )}
      </div>
    </div>
        ...
        <div className="w-full px-4 flex flex-col md:flex-row justify-between items-center gap-4 max-w-screen-2xl mx-auto">
            ...
            <div className="flex items-center gap-3 self-end md:self-auto">
              <div className="font-display flex items-center gap-2 flex-wrap text-xs sm:text-sm font-bold tracking-widest uppercase">
                  ...
                  <span className="text-amber-900/40">/</span>
                  <span className="text-red-800">Dádivas de Aharadak</span>
              </div>
              <ThemeToggle />
            </div>
        </div>
      ...
      <main className="relative z-10 w-full px-6 py-12 max-w-screen-2xl mx-auto">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <p className="text-sm text-amber-950/70 italic font-bold">
                  Clique para expandir informações sobre parceiros e montarias.
                </p>
              </div>
            </div>
          ...
          <div className={`overflow-hidden transition-all duration-500 ease-in-out border-x-2 border-b-2 border-amber-900/30 rounded-b-xl bg-[rgb(var(--bg-inset-rgb))] ${isIntroOpen ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0 border-transparent'}`}>
            <div className="p-5 md:p-10 font-serif text-amber-950/85 text-base md:text-lg text-left md:text-justify leading-relaxed flex flex-col gap-6">
              ...
              <section>
                ...
                <p className="font-medium text-lg leading-relaxed mb-6">
                 Cada simbionte ocupa o espaço de um item vestido e conta como dois poderes da Tormenta. Sempre que um simbionte é implantado, o hospedeiro faz um teste de Constituição (CD 10, +1 para cada outro poder da Tormenta que possuir). Se passar, o simbionte funciona normalmente. Se falhar, entretanto, a ligação não surte os efeitos desejados; o simbionte ocupa o espaço de um item vestido e causa a perda de 1 ponto de Carisma. Não existem meios conhecidos de remover um simbionte.
                </p>
                <p className="font-medium text-lg leading-relaxed mb-6">
                 Apesar de sua origem sinistra, existe mercado para simbiontes em suas formas larvais, ainda não unidos a hospedeiros. São considerados itens valiosos, que não podem ser roubados (nem removidos). Aventureiros realizam expedições em busca de ninhos onde costumam ser encontrados. Mas é sempre raro achá-los à venda, mesmo em lugares como Vectora.
                </p>
              </section>
        ...
        <section className="w-full">
            <h2 className="font-display text-3xl font-bold text-red-800 mb-6 flex items-center gap-3 tracking-wide">
                <span className="text-red-800 text-3xl">❖</span> Acervo de dádivas
            </h2>
            <GearFilterableTable allGear={aharadak} />
        </section>
      </main>
