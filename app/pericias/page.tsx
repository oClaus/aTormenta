...
const SkillAccordionItem = ({ skill, isSearching }: { skill: Skill; isSearching: boolean }) => {
  ...
  return (
    <div className="border-2 border-amber-900/30 rounded-xl bg-[#e8dac1] overflow-hidden shadow-sm transition-all duration-300 hover:border-red-800/50 hover:shadow-[0_4px_20px_rgba(153,27,27,0.15)] scroll-mt-24 font-serif">
      ...
      >
        <div className="text-left">
          <h2 className="text-xl md:text-2xl font-bold text-red-800 transition-colors drop-shadow-sm tracking-wide">
            {skill.name}
          </h2>
          <p className="text-[10px] md:text-xs text-amber-950/60 mt-1 uppercase tracking-widest font-bold">
      ...
      >
        <div className="overflow-hidden">
          <div className="p-5 md:p-8 bg-[#fbf5e6] border-t-2 border-amber-900/20 space-y-6 md:space-y-8">
            {/* Descrição Geral */}
            ...
            <div className="space-y-4">
              {skill.functions.map((func: SkillFunction, index: number) => (
                <div key={index} className="bg-[#e8dac1]/50 p-5 md:p-6 rounded-xl border border-amber-900/20 shadow-sm hover:border-red-800/30 transition-colors">
                  <h4 className="text-lg font-bold text-red-800 mb-2 flex items-start gap-2 tracking-wide uppercase">
                  ...
                  {func.table && (
                    <div className="mt-4 mb-4 overflow-hidden rounded-lg border border-amber-900/20 md:ml-5 bg-[#fbf5e6] shadow-sm">
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr>
                            {func.table.headers.map((header, idx) => (
                              <th key={idx} className="p-2 md:p-3 font-bold uppercase tracking-widest text-sm text-center text-red-800 border-b-2 border-amber-900/20">
                                {header}
                              </th>
                            ))}
                          </tr>
                        ...
                        <tbody>
                          {func.table.rows.map((row, rowIdx) => (
                            <tr key={rowIdx} className="text-amber-950/85 text-sm md:text-base even:bg-[#e8dac1]/40 odd:bg-transparent">
          ...
          <div className={`overflow-hidden transition-all duration-500 ease-in-out border-x-2 border-b-2 border-amber-900/30 rounded-b-xl bg-[#fbf5e6] ${isRulesOpen ? 'max-h-[5000px] opacity-100' : 'max-h-0 opacity-0 border-transparent'}`}>
            <div className="p-5 md:p-10 font-serif text-amber-950/85 text-base md:text-lg text-left md:text-justify leading-relaxed flex flex-col gap-8">
              ...
              <section className="border-t-2 border-amber-900/20 pt-8">
                  <h2 className="text-2xl md:text-3xl font-bold text-red-800 mb-4 tracking-wide border-b-2 border-amber-900/10 pb-2">
                  Escolhendo Perícias
                  </h2>
              ...
              <section className="border-t-2 border-amber-900/20 pt-8">
                  ...
                  <div className="p-6 bg-[#fbf5e6] rounded-xl border border-amber-900/20 shadow-sm border-l-4 border-l-red-800">
