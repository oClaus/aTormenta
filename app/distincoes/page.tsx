...
const formatTextWithBreaks = (text: string) => {
  ...
  return lines.map((line, index) => {
    let formattedLine = line
      .replace(/\*\*\*(.*?)\*\*\*/g, '<em class="text-red-800 font-bold">$1</em>') // Negrito e Itálico (Red-800)
      .replace(/\*\*(.*?)\*\*/g, '<strong class="text-amber-950/90">$1</strong>') // Negrito
      .replace(/\*(.*?)\*/g, '<em class="text-amber-900/80 font-medium">$1</em>') // Itálico
      .replace(/### (.*)/g, '<h3 class="text-2xl font-bold text-red-800 mt-6 mb-3 font-serif border-b-2 border-amber-900/10 pb-2 tracking-wide">$1</h3>') // Subtítulos
      .replace(/> (.*)/g, '<blockquote class="border-l-4 border-red-800 pl-4 py-3 my-4 text-base italic text-amber-950/85 bg-[#e8dac1]/50 rounded-r-xl font-medium shadow-sm">$1</blockquote>');
...
const RulesSection = () => {
  ...
  return (
    <div className="mb-12 w-full">
      ...
      <div className={`overflow-hidden transition-all duration-500 ease-in-out border-x-2 border-b-2 border-amber-900/30 rounded-b-xl bg-[#fbf5e6] ${isOpen ? 'max-h-[8000px] opacity-100' : 'max-h-0 opacity-0 border-transparent'}`}>
        <div className="p-5 md:p-10 font-serif text-amber-950/85 text-base md:text-lg text-left md:text-justify leading-relaxed flex flex-col gap-8 font-medium">
          ...
          <section className="border-t-2 border-amber-900/20 pt-8">
            <h3 className="text-2xl md:text-3xl font-bold text-red-800 mb-4 tracking-wide border-b-2 border-amber-900/10 pb-2">Distinções em jogo</h3>
            <p className="mb-4">
          ...
          {/* Admissão */}
          <section className="border-t-2 border-amber-900/20 pt-8">
            <h3 className="text-2xl md:text-3xl font-bold text-red-800 mb-4 tracking-wide border-b-2 border-amber-900/10 pb-2">Admissão</h3>
            <p className="mb-4">
          ...
          {/* Marca da Distinção */}
          <section className="border-t-2 border-amber-900/20 pt-8">
            <h3 className="text-2xl md:text-3xl font-bold text-red-800 mb-4 tracking-wide border-b-2 border-amber-900/10 pb-2">Marca da Distinção</h3>
            <p>
          ...
          <section className="border-t-2 border-amber-900/20 pt-8">
            <h3 className="text-2xl md:text-3xl font-bold text-red-800 mb-4 tracking-wide border-b-2 border-amber-900/10 pb-2">Poderes de distinção</h3>
            <p className="mb-4">
          ...
          {/* Usando distinções */}
          <section className="border-t-2 border-amber-900/20 pt-8">
            <h3 className="text-2xl md:text-3xl font-bold text-red-800 mb-4 tracking-wide border-b-2 border-amber-900/10 pb-2">Usando distinções</h3>
            <p className="mb-4">
          ...
          {/* Aplicando admissões */}
          <section className="border-t-2 border-amber-900/20 pt-8">
            <h3 className="text-2xl md:text-3xl font-bold text-red-800 mb-4 tracking-wide border-b-2 border-amber-900/10 pb-2">Aplicando admissões</h3>
            <p className="mb-4">
          ...
          {/* E agora? */}
          <section className="border-t-2 border-amber-900/20 pt-8">
            <h3 className="text-2xl md:text-3xl font-bold text-red-800 mb-4 tracking-wide border-b-2 border-amber-900/10 pb-2">E agora?</h3>
            <p className="mb-4">
...
// --- Componente Modal de Detalhes da Distinção (Visual Livro) ---
const DistinctionModal = ({ distinction, onClose }: { distinction: Distinction, onClose: () => void }) => {
  ...
  return (
    ...
    >
      {/* Container Principal */}
      <div className="bg-[#f5e6d0] rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto relative border-4 border-double border-amber-900/40 font-serif my-8 md:my-0 custom-scrollbar">
        
        {/* Header do Modal */}
        <div className="sticky top-0 z-20 p-6 bg-[#e8dac1]/95 border-b-2 border-amber-900/20 flex justify-between items-center backdrop-blur shadow-sm">
          <div>
              <h2 className="text-3xl font-bold text-red-800 tracking-wide uppercase">
                {distinction.name}
              </h2>
              <p className="text-amber-950/70 text-[10px] md:text-xs font-bold uppercase tracking-widest mt-1">
                Origem: {distinction.origin}
              </p>
          </div>
          <button 
            onClick={onClose} 
            className="text-amber-950/40 hover:text-red-800 transition-colors text-4xl leading-none pb-2"
            title="Fechar"
          >
            &times;
          </button>
        ...
        {/* Conteúdo da Distinção */}
        <div className="p-6 md:p-10 flex flex-col gap-10 text-amber-950 bg-[radial-gradient(circle_at_center,transparent_0%,rgba(69,26,3,0.05)_100%)]">
          
          <div className="space-y-10">
            {/* Imagem da Distinção no MODAL */}
            {distinction.image && (
              <div className="relative w-full h-80 md:h-96 rounded-xl overflow-hidden shadow-md border-2 border-amber-900/20 bg-[#fbf5e6]">
                <Image 
                  src={distinction.image} 
                  alt={distinction.name} 
                  layout="fill" 
                  objectFit="contain" 
                  className="transition-transform duration-700 mix-blend-multiply filter sepia-[0.3] p-4"
                />
              </div>
            )}

            {/* Introdução */}
            <div className="bg-[#fbf5e6] p-8 rounded-xl border border-amber-900/20 shadow-sm border-l-4 border-l-red-800">
              <h2 className="text-2xl font-bold text-amber-950 mb-4 border-b-2 border-amber-900/10 pb-2 uppercase tracking-widest flex items-center gap-2">
                <span className="opacity-70 text-3xl">📜</span> Introdução
              </h2>
            ...
            {/* Admissão */}
            <div>
              <h2 className="text-2xl font-bold text-red-800 mb-4 border-b-2 border-amber-900/20 pb-2 tracking-wide flex items-center gap-3">
                <span className="text-red-800/60 text-3xl">🗝️</span> Admissão
              </h2>
            ...
            {/* Marca da Distinção */}
            {distinction.mark && (
                <div className="bg-[#e8dac1]/50 p-8 rounded-xl border border-amber-900/20 shadow-sm">
                <h2 className="text-2xl font-bold text-red-800 mb-4 border-b-2 border-amber-900/10 pb-2 tracking-wide flex items-center gap-3">
                    <span className="text-red-800/60 text-3xl">✧</span> Marca da Distinção
                </h2>
            ...
            <div>
              <h2 className="text-2xl font-bold text-red-800 mb-6 border-b-2 border-amber-900/20 pb-2 tracking-wide flex items-center gap-3">
                <span className="text-red-800/60 text-3xl">⚡</span> Poderes da Distinção
              </h2>
              <div className="space-y-6">
                {distinction.powers.map((power, index) => (
                  <div key={index} className="bg-[#fbf5e6] border-2 border-amber-900/10 p-6 rounded-xl hover:border-red-800/30 transition-colors shadow-sm">
                    <h3 className="text-xl font-bold text-amber-950 mb-4 border-b-2 border-amber-900/10 pb-2">
                        {power.name}
                    </h3>
          ...
          {distinction.extras && (
            <div className="mt-8 pt-8 border-t-4 border-double border-amber-900/20">
                <div className="bg-[#e8dac1] p-8 rounded-xl border-2 border-amber-900/20 shadow-md relative overflow-hidden">
                    <h2 className="text-2xl font-bold text-red-800 mb-6 border-b-2 border-amber-900/10 pb-2 tracking-wide relative z-10 flex items-center gap-3">
                        <span className="text-red-800/60 text-3xl">📝</span> Notas & Extras
                    </h2>
...
const DistinctionCard = ({ distinction, onClick }: { distinction: Distinction, onClick: (distinction: Distinction) => void }) => {
  return (
    <button onClick={() => onClick(distinction)} className="w-full h-full group text-left outline-none">
      <div className="h-full bg-[#e8dac1] border-2 border-amber-900/20 rounded-xl overflow-hidden shadow-sm hover:border-red-800/40 hover:shadow-[0_4px_20px_rgba(153,27,27,0.15)] transition-all duration-300 hover:-translate-y-1 flex flex-col relative">
        ...
        {/* Imagem com fundo pergaminho claro e efeito sépia */}
        <div className="relative w-full h-56 bg-[#fbf5e6] border-b-2 border-amber-900/10 overflow-hidden flex items-center justify-center">
            {distinction.image ? (
            <Image 
                src={distinction.image} 
                alt={distinction.name} 
                layout="fill" 
                objectFit="contain" 
                className="transition-transform duration-500 group-hover:scale-105 mix-blend-multiply filter sepia-[0.2] opacity-90 p-4"
            />
            ) : (
                <div className="w-full h-full flex flex-col items-center justify-center text-amber-900/20 bg-[#fbf5e6]">
                    <span className="text-6xl mb-2">🎖️</span>
                    <span className="text-xs font-bold uppercase tracking-widest font-serif">Sem Ilustração</span>
                </div>
            )}
        </div>

        {/* Conteúdo */}
        <div className="p-6 flex-1 flex flex-col bg-[#e8dac1]">
          <div className="mb-4">
            <h3 className="text-xl md:text-2xl font-bold text-red-800 group-hover:text-red-700 transition-colors font-serif tracking-wide">
                {distinction.name}
            </h3>
          </div>
          
          <div className="mt-auto pt-4 border-t-2 border-amber-900/10 text-right">
            <span className="inline-block px-2 py-1 rounded bg-[#fbf5e6] border border-amber-900/20 text-[10px] uppercase tracking-widest text-amber-950/70 font-bold shadow-sm">
               {distinction.origin}
            </span>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-[#f5e6d0] text-amber-950 font-serif selection:bg-amber-800 selection:text-amber-50 relative overflow-x-hidden bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[#f5e6d0] to-[#e6d5b8]">
      ...
      <main className="relative z-10 w-full px-6 py-12 max-w-screen-2xl mx-auto">
        ...
        <div className="mb-12 p-6 rounded-xl bg-[#e8dac1] border-2 border-amber-900/30 shadow-[inset_0_2px_10px_rgba(0,0,0,0.05)] w-full">
            <label className="block text-sm font-bold text-amber-950/70 mb-3 uppercase tracking-widest font-serif">
                Buscar Distinção
            </label>
            <div className="relative">
                ...
                className="w-full px-5 py-3 pr-12 bg-[#fbf5e6] border-2 border-amber-900/20 rounded-lg text-amber-950/85 placeholder-amber-900/40 focus:outline-none focus:border-red-800/50 focus:ring-1 focus:ring-red-800/50 transition-all font-serif shadow-sm"
                />
                {searchTerm ? (
                  <button 
        ...
        {/* Grid de Distinções limitando a 3 ou 4 colunas no máximo */}
