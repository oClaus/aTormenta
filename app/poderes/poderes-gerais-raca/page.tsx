...
import { powersGeraisRaca } from "@/data/powers-gerais-raca";
import { formatOrigin } from "@/types/power";

export default function GeraisPodersPage() {
  const [searchTerm, setSearchTerm] = useState("");

  const filteredPowers = powersGeraisRaca
    .filter(
      (power) =>
    ...
    <div className="min-h-screen bg-stone-950 text-stone-200 font-serif selection:bg-red-900 selection:text-white relative overflow-x-hidden">
      ...
      <header className="relative z-10 w-full p-6 border-b-2 border-stone-800 bg-stone-950/90 backdrop-blur-md shadow-lg mb-8 md:mb-12">
        <div className="w-full px-4 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-2 flex-wrap text-xs sm:text-sm font-bold tracking-wide uppercase self-end md:self-auto">
                <Link href="/" className="text-stone-500 hover:text-orange-600 transition-colors whitespace-nowrap">
                  Início
                </Link>
                <span className="text-stone-700">/</span>
                <Link href="/poderes" className="text-stone-500 hover:text-orange-600 transition-colors">
                  Poderes
                </Link>
                <span className="text-stone-700">/</span>
                <span className="text-red-700">Poderes Gerais - Raça</span>
            </div>
        </div>
      </header>
      ...
      <div className="relative z-10 w-full px-4 sm:px-8 md:px-12 pb-12">
        ...
        <div className="mb-12 p-8 bg-stone-900/50 rounded border border-stone-800 w-full">
            <h1 className="text-4xl sm:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-orange-700 via-amber-500 to-orange-700 mb-6 drop-shadow-md">
            Poderes Gerais - Raça
            </h1>
            <p className="text-stone-300 text-lg font-serif leading-relaxed">
            Poderes de raça são um novo grupo de poderes gerais. São caracterizados por possuírem como pré-requisito pertencer a uma raça específica. Exceto por isso, funcionam como quaisquer outros poderes gerais.
            </p>
        </div>
        ...
        <div className="mb-8 p-6 rounded bg-stone-900 border border-stone-800 shadow-inner w-full">
            ...
            <div className="relative">
                ...
                className="w-full px-5 py-3 bg-stone-950 border border-stone-700 rounded text-stone-200 placeholder-stone-600 focus:outline-none focus:border-orange-600 focus:ring-1 focus:ring-orange-900 transition-all font-serif"
                />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 items-stretch mb-16">
            ...
            <div
                key={power.id}
                className="rounded-xl bg-stone-900 border border-stone-800 p-6 hover:border-orange-900/50 hover:shadow-[0_0_20px_rgba(20,184,166,0.15)] transition-all duration-300 flex flex-col h-full group hover:-translate-y-1"
            >
                {/* Nome do Poder */}
                <h3 className="text-xl font-bold text-orange-700 mb-3 group-hover:text-orange-400 transition-colors font-serif">
                {power.name}
                </h3>
                ...
                {power.prerequisite && (
                    ...
                    <p className="text-xs text-stone-500 font-serif">
                        <span className="font-bold text-orange-700 uppercase tracking-wide">Pré-requisito:</span> {power.prerequisite}
                    </p>
                    </div>
                )}
