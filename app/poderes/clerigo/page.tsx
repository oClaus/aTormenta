        ...
        <div className="mb-12 w-full">
          ...
          <div className={`overflow-hidden transition-all duration-500 ease-in-out border-x-2 border-b-2 border-amber-900/30 rounded-b-xl bg-[rgb(var(--bg-inset-rgb))] ${isIntroOpen ? 'max-h-[3000px] opacity-100' : 'max-h-0 opacity-0 border-transparent'}`}>
            <div className="p-5 md:p-10 font-serif text-amber-950/85 text-base md:text-lg text-left md:text-justify leading-relaxed flex flex-col gap-8">
              ...
              {/* Seção 1: Missas */}
              <section>
                <h3 className="font-display text-2xl font-bold text-red-800 mb-4 tracking-wide border-b-2 border-amber-900/20 pb-2">Missas</h3>
                <p className="font-medium mb-6">Alguns poderes do clérigo são Missas. Esses poderes compartilham as seguintes regras:</p>

                <ul className="space-y-4 mb-6">
                  <li className="flex items-start gap-3">
                    <span className="text-[10px] text-red-800/60 mt-2">◆</span>
                    <span className="font-medium">Rezar uma Missa exige uma hora e o gasto de materiais especiais (como velas, incensos, água benta...) no valor de T$ 25. Um mesmo celebrante pode aplicar mais de um poder de Missa na mesma celebração. Cada poder adicional aumenta o custo da Missa em T$ 25.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-[10px] text-red-800/60 mt-2">◆</span>
                    <span className="font-medium">Uma Missa afeta um número máximo de pessoas igual a 1 + sua Sabedoria. Todas as pessoas precisam estar presentes durante toda a Missa.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-[10px] text-red-800/60 mt-2">◆</span>
                    <span className="font-medium">Os efeitos de uma Missa duram um dia. Uma mesma criatura só pode receber os benefícios da mesma Missa uma vez por dia.</span>
                  </li>
                </ul>
              </section>

              {/* Seção 2: Mestre Celebrante */}
              <section className="border-t-2 border-amber-900/20 pt-8">
                <h3 className="font-display text-2xl font-bold text-red-800 mb-4 tracking-wide">Mestre Celebrante</h3>
                <p className="font-medium mb-6">
                  O poder <span className="text-red-800 font-bold uppercase tracking-wider text-sm">Mestre Celebrante</span> dobra os benefícios de Missas celebradas pelo personagem. Abaixo detalhamos como isso se aplica às novas missas:
                </p>

                <ul className="space-y-4 mb-6">
                  <li className="flex items-start gap-3">
                    <span className="text-[10px] text-red-800/60 mt-2">◆</span>
                    <span className="font-medium">Se a Missa fornece um bônus numérico que se aplica por toda sua duração, como o de <span className="font-bold text-amber-950 italic">Mente Abençoada</span>, esse bônus é dobrado.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-[10px] text-red-800/60 mt-2">◆</span>
                    <span className="font-medium">Se fornece uma quantidade de PV ou PM, como <span className="font-bold text-amber-950 italic">Elevação do Espírito</span>, essa quantidade é dobrada.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-[10px] text-red-800/60 mt-2">◆</span>
                    <span className="font-medium">Se permite usar uma habilidade ou receber um benefício uma vez, como <span className="font-bold text-amber-950 italic">Compartilhar Milagre</span>, esse efeito pode ser usado uma vez adicional pela duração da Missa.</span>
                  </li>
                </ul>
              </section>
        ...
        {filteredPowers.length === 0 && (
          <div className="text-center py-20 border-2 border-dashed border-amber-900/30 rounded-xl bg-[rgb(var(--bg-card-rgb))]/50 flex flex-col items-center justify-center mt-8">
            ...
            <p className="font-display text-amber-950/70 text-lg italic tracking-wide">
              Nenhum poder de clérigo encontrado.
            </p>
          </div>
        )}
      </main>
