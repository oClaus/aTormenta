...
import { powersGeraisMagia } from "@/data/powers-gerais-magia";
import { formatOrigin } from "@/types/power";

export default function GeraisPodersPage() {
  const [searchTerm, setSearchTerm] = useState("");

  const filteredPowers = powersGeraisMagia
    .filter(
      (power) =>
    ...
    <div className="min-h-screen bg-stone-950 text-stone-200 font-serif selection:bg-red-900 selection:text-white relative overflow-x-hidden">
      ...
      <header className="relative z-10 w-full p-6 border-b-2 border-stone-800 bg-stone-950/90 backdrop-blur-md shadow-lg mb-8 md:mb-12">
        <div className="w-full px-4 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-2 flex-wrap text-xs sm:text-sm font-bold tracking-wide uppercase self-end md:self-auto">
                ...
                <Link href="/poderes" className="text-stone-500 hover:text-fuchsia-600 transition-colors">
                  Poderes
                </Link>
                <span className="text-stone-700">/</span>
                <span className="text-red-700">Poderes Gerais - Magia</span>
            </div>
        </div>
      </header>
      ...
      <div className="relative z-10 w-full px-4 sm:px-8 md:px-12 pb-12">
        ...
        <div className="mb-12 p-8 bg-stone-900/50 rounded border border-stone-800 w-full">
            <h1 className="text-4xl sm:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-500 via-violet-500 to-fuchsia-500 mb-6 drop-shadow-md">
            ...
            <h2 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 via-violet-400 to-fuchsia-400 mb-3 drop-shadow-sm">
            Poderes de Aprimoramento
            </h2>
            <p className="text-stone-400 font-serif leading-relaxed">
            Estes poderes acrescentam melhorias às magias conhecidas pelo conjurador. Eles seguem todas as regras para aprimoramentos. Você pode aplicar quantos aprimoramentos quiser, desde que não ultrapasse seu limite de PM.
            </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 items-stretch mb-16">
            ...
            <div
                key={power.id}
                className="rounded-xl bg-stone-900 border border-stone-800 p-6 hover:border-fuchsia-900/50 hover:shadow-[0_0_20px_rgba(192,38,211,0.15)] transition-all duration-300 flex flex-col h-full group hover:-translate-y-1"
            >
                {/* Nome do Poder */}
                <h3 className="text-xl font-bold text-fuchsia-500 mb-3 group-hover:text-fuchsia-400 transition-colors font-serif">
                {power.name}
                </h3>
                ...
                {power.prerequisite && (
                    ...
                    <p className="text-xs text-stone-500 font-serif">
                        <span className="font-bold text-fuchsia-700 uppercase tracking-wide">Pré-requisito:</span> {power.prerequisite}
                    </p>
                    </div>
                )}
