...
const extractPmCost = (text: string): number | null => {
  const match = text.match(/\((\d+)\s*PM\)/i);
...
function ConditionTag({ conditionId, onRemove }: { conditionId: string; onRemove: () => void }) {
  ...
  return (
    ...
    >
      ...
      {open && (
        <div className="absolute bottom-full left-0 mb-1 z-50 w-64 bg-[#2a1810] text-[#fbf5e6] text-xs rounded-xl p-3 shadow-2xl border border-amber-900/40 pointer-events-none">
          <p className="font-bold text-amber-400 mb-1">{cond.name}</p>
          {cond.efeito && (
            <p className="text-amber-400/50 text-[10px] mb-1.5 uppercase tracking-widest">{cond.efeito}</p>
          )}
...
function ConditionPicker({
  ...
  const grouped = allConditions.reduce<Record<string, Condition[]>>((acc, c) => {
    acc[c.origin] = acc[c.origin] ?? [];
    acc[c.origin].push(c);
    return acc;
  }, {});
  ...
  return (
    ...
    >
      <div
        className="bg-[#f5e6d0] border-2 border-amber-900/40 rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-5 border-b-2 border-amber-900/20 flex items-center justify-between gap-3">
          <h2 className="text-xl font-bold text-red-800 font-serif tracking-wide">Condições</h2>
          <button onClick={onClose} className="text-amber-950/40 hover:text-red-800 text-2xl leading-none">✕</button>
        </div>
        <div className="p-4 border-b border-amber-900/15">
          <input
            autoFocus
            type="text"
            ...
            className="w-full px-4 py-2 bg-[#fbf5e6] border-2 border-amber-900/25 rounded-xl text-amber-950 placeholder-amber-900/40 font-serif text-sm focus:outline-none focus:border-red-800/60"
          />
        </div>
        <div className="overflow-y-auto p-4 space-y-5 custom-scrollbar">
          {Object.entries(grouped).map(([origin, list]) => {
            ...
            return (
              <div key={origin}>
                <p className="text-[10px] uppercase tracking-widest text-amber-950/40 font-bold mb-2.5">{origin}</p>
                <div className="flex flex-wrap gap-2">
                  {filtered.map((c) => {
                    const isActive = active.includes(c.id);
                    const isPos = POSITIVE_CONDITIONS.has(c.id);
                    return (
                      <div key={c.id} className="relative inline-block group">
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-full text-xs font-bold border-2 transition-all ${
                            isActive
                              ...
                              : "bg-[#fbf5e6] border-amber-900/20 text-amber-950/70 hover:border-amber-700/50"
                          }`}
                        >
                          {isActive ? "✓ " : ""}{c.name}
                        </button>
                        <div className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 w-60 bg-[#2a1810] text-[#fbf5e6] text-xs rounded-xl p-3 shadow-2xl border border-amber-900/40 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                          <p className="font-bold text-amber-400 mb-1">{c.name}</p>
                          {c.efeito && (
                            <p className="text-amber-400/50 text-[10px] mb-1.5 uppercase tracking-widest">{c.efeito}</p>
                          )}
                          <p className="leading-relaxed text-[#fbf5e6]/80 whitespace-pre-line">{c.description}</p>
                        </div>
                      </div>
                    );
                  })}
        ...
        <div className="p-5 border-t-2 border-amber-900/20 flex gap-3 justify-end">
          ...
          <button
            onClick={() => {
              onSave({
                ...monster,
                threat: draft as Threat,
                pvMax,
                pvCurrent: Math.min(monster.pvCurrent, pvMax),
                pmMax,
                pmCurrent: Math.min(monster.pmCurrent, pmMax),
              });
              onClose();
            }}
...
function ParticipantCard({
  ...
  const toggleCondition = (condId: string) => {
    ...
    if (!cond) return;
    onLog(has
      ? `🔓 ${participant.name}: "${cond.name}" removida`
      : `🔒 ${participant.name}: "${cond.name}" aplicada`);
    onUpdate({
      ...participant,
      conditions: has
        ? participant.conditions.filter((c) => c !== condId)
        : [...participant.conditions, condId],
    });
  };
  ...
  return (
    <>
      <div className={`relative bg-[#f5e6d0] border-2 rounded-2xl transition-all duration-300 font-serif overflow-hidden ${
        isActive
            ...
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <span className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-sm font-black border-2 ${
                isActive
                  ...
                  : "bg-[#e8dac1] border-amber-900/30 text-amber-950/70"
              }`}>
                {isMonster ? "☠" : "⚔"}
              </span>
              <div className="min-w-0">
                ...
                {m && (
                  <p className="text-xs text-amber-950/50">
                    {m.threat.tipo} · ND {m.threat.nd} · DEF {m.threat.defesa}
                    {m.threat.fort !== undefined && ` · Fort +${m.threat.fort}`}
                    {m.threat.ref !== undefined && ` · Ref +${m.threat.ref}`}
                  </p>
                )}
              </div>
            </div>
            <div className="flex items-center gap-1.5 shrink-0">
              <span className="text-xs text-amber-950/50 font-bold bg-[#e8dac1] px-2 py-1 rounded-lg border border-amber-900/20">
                INI {participant.initiativeRoll}
              </span>
              {isMonster && onEdit && (
                <button
                  onClick={onEdit}
                  title="Editar criatura"
                  className="w-7 h-7 flex items-center justify-center rounded-lg border-2 border-amber-900/20 bg-[#e8dac1] text-amber-950/50 hover:text-red-800 hover:border-red-800/40 transition-all text-xs"
                >✎</button>
              )}
              <button
                onClick={onRemove}
                title="Remover"
                className="w-7 h-7 flex items-center justify-center rounded-lg border-2 border-amber-900/20 bg-[#e8dac1] text-amber-950/40 hover:text-red-800 hover:border-red-800/40 transition-all text-xs"
              >✕</button>
            </div>
          </div>
          ...
          {m && (
            <div className="flex gap-2 flex-wrap mb-3">
              <div className="flex items-center gap-1.5">
                <input
                  type="number" min="0" placeholder="Dano" value={dmgInput}
                  onChange={(e) => setDmgInput(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && applyDamage()}
                  className={ic}
                />
                <button onClick={applyDamage} className="px-3 py-1.5 bg-red-800 text-[#fbf5e6] rounded-lg text-xs font-black hover:bg-red-900 transition-all">
                  Dano
                </button>
              </div>
              <div className="flex items-center gap-1.5">
                <input
                  type="number" min="0" placeholder="Cura" value={healInput}
                  onChange={(e) => setHealInput(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && applyHeal()}
                  className={ic}
                />
                <button onClick={applyHeal} className="px-3 py-1.5 bg-emerald-700 text-white rounded-lg text-xs font-black hover:bg-emerald-800 transition-all">
                  Curar
                </button>
              </div>
              <button
                onClick={() => setShowCondPicker(true)}
                className="px-3 py-1.5 rounded-lg border-2 border-amber-900/25 text-amber-950/60 text-xs font-bold hover:border-amber-700/50 transition-all"
              >
                + Condição
              </button>
            </div>
                  ...
                  return (
                    <div key={i} className="flex items-start gap-2 group">
                      <button
                        ...
                        title={
                          cost !== null
                            ? canUse ? `Usar (−${cost} PM)` : "PM insuficiente"
                            : "Registrar uso"
                        }
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {rest && (
                          <p className="text-[11px] text-amber-950/50 leading-snug mt-0.5 line-clamp-2 group-hover:line-clamp-none transition-all">
                            {rest}
                          </p>
                        )}
                      </div>
          ...
          {m && (m.threat.ataqueCorpoACorpo || m.threat.ataqueDistancia) && (
            <div className="border-t border-amber-900/15 pt-3 mt-3">
              ...
              {m.threat.ataqueCorpoACorpo && (
                <p className="text-xs text-amber-950/75 mb-1">
                  <span className="text-red-800 font-bold">CaC: </span>{m.threat.ataqueCorpoACorpo}
                </p>
              )}
              {m.threat.ataqueDistancia && (
                <p className="text-xs text-amber-950/75">
                  <span className="text-red-800 font-bold">Dist: </span>{m.threat.ataqueDistancia}
                </p>
              )}
            </div>
          )}
  ...
  return (
    <div className="flex flex-col gap-3">
      <input
        ...
        value={search}
        onChange={(e) => setSearch(e.target.value)}
        className="w-full px-4 py-2.5 bg-[#fbf5e6] border-2 border-amber-900/30 rounded-xl text-amber-950 placeholder-amber-900/40 font-serif focus:outline-none focus:border-red-800/60 text-sm"
      />
      <div className="max-h-64 overflow-y-auto space-y-1.5 pr-1 custom-scrollbar">
        {filtered.map((t) => (
          <button
            ...
            className={`w-full text-left px-4 py-2.5 rounded-xl border-2 font-serif transition-all flex items-center gap-3 text-sm ${
              selected.has(t.id)
                ? "bg-red-800/10 border-red-800/50 text-red-900"
                : "bg-[#fbf5e6] border-amber-900/20 text-amber-950/80 hover:border-amber-700/40"
            }`}
          >
            {selected.has(t.id) && <span className="text-red-800 shrink-0">✓</span>}
            <span className="font-bold flex-1 truncate">{t.name}</span>
            <span className="text-xs opacity-50 shrink-0">ND {t.nd} · {t.tipo}</span>
          </button>
        ))}
        {!filtered.length && (
          <p className="text-center text-amber-950/40 italic py-6 text-sm">Nenhuma criatura encontrada.</p>
        )}
      </div>
    </div>
  );
...
export default function CombatePage() {
  ...
  const addPlayer = () => {
    ...
    setParticipants((prev) => [
      ...prev,
      {
        ...
        type: "player",
        initiativeRoll: parseInt(newPlayerIni) || 0,
        conditions: [],
      },
    ]);
    setNewPlayerName("");
  ...
  const goToRolling = () => {
    const monsters: MonsterParticipant[] = Array.from(selectedThreatIds).map((id) => {
      const t = allThreats.find((x) => x.id === id)!;
      return {
        id: uid(),
        name: t.name,
        type: "monster",
        initiativeRoll: t.iniciativa,
        threat: t,
        pvMax: t.pv,
        pvCurrent: t.pv,
        pmMax: t.pm ?? 0,
    ...
    const all = [...participants, ...monsters];
    setParticipants(all);
  ...
  return (
    <div className="min-h-screen bg-[#f5e6d0] text-amber-950 font-serif selection:bg-amber-800 selection:text-amber-50 relative overflow-x-hidden bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[#f5e6d0] to-[#e6d5b8]">
      ...
      <header className="relative z-10 w-full p-6 border-b-4 border-double border-amber-900/40 bg-[#e8dac1]/90 backdrop-blur-md shadow-sm">
        <div className="w-full px-2 sm:px-6 md:px-12 lg:px-20 xl:px-28 flex flex-col md:flex-row justify-between items-center gap-4">
          <Link href="/" className="inline-block group">
            <h1
              className="text-4xl font-bold tracking-widest uppercase text-transparent bg-clip-text bg-gradient-to-b from-red-700 via-red-800 to-red-950 drop-shadow-sm transition-all group-hover:brightness-125"
      ...
      <main className="relative z-10 w-full px-4 sm:px-8 md:px-12 lg:px-20 xl:px-28 py-8">
        ...
        {phase === "setup" && (
          <div className="max-w-4xl mx-auto">
            <div className="mb-10">
              <h2 className="text-4xl sm:text-5xl font-bold text-red-800 mb-3 border-b-2 border-amber-900/20 pb-4 tracking-wider drop-shadow-sm">
                Preparar Combate
              </h2>
              <p className="text-amber-950/70 italic text-lg border-l-4 border-red-800/60 pl-4 font-medium">
                Adicione os jogadores e selecione as criaturas para o encontro.
              </p>
            </div>

            <div className="grid md:grid-cols-2 gap-6">
              {/* Jogadores — só nome e iniciativa */}
              <div className="bg-[#e8dac1] border-2 border-amber-900/30 rounded-2xl p-5 shadow-md">
                ...
                <p className="text-xs text-amber-950/40 italic mb-4 border-b-2 border-amber-900/20 pb-3">
                  Apenas nome para rastrear a ordem de iniciativa.
                </p>
                <div className="space-y-2 mb-4">
                  ...
                  <input
                    type="number"
                    placeholder="Resultado da Iniciativa (d20 + bônus)"
                    value={newPlayerIni}
                  ...
                  <button
                    onClick={addPlayer}
                    className="w-full py-2 bg-amber-900 text-[#fbf5e6] rounded-xl font-bold text-sm hover:bg-amber-950 transition-all shadow-sm"
                  >
                    + Adicionar Jogador
                  </button>
                </div>
                  ...
                  {playersAdded.map((p) => (
                    <div key={p.id} className="flex items-center justify-between px-3 py-2 bg-[#fbf5e6] rounded-xl border border-amber-900/20 text-sm">
                      ...
                      <button
                        ...
                        className="text-amber-900/40 hover:text-red-800 text-xs ml-2"
                      >✕</button>
                    </div>
                  ))}
                  {!playersAdded.length && (
                    <p className="text-center text-amber-950/30 italic text-sm py-4">Nenhum jogador adicionado.</p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-[#e8dac1] border-2 border-amber-900/30 rounded-2xl p-5 shadow-md">
                <h3 className="text-xl font-bold text-red-800 mb-1 tracking-wide">
                  ☠ Criaturas {selectedThreatIds.size > 0 && `(${selectedThreatIds.size})`}
                </h3>
                <p className="text-xs text-amber-950/40 italic mb-4 border-b-2 border-amber-900/20 pb-3">
                  Pode selecionar a mesma criatura múltiplas vezes depois.
                </p>
            ...
            <div className="mt-8 flex justify-center">
              <button
                onClick={goToRolling}
                disabled={playersAdded.length === 0 && selectedThreatIds.size === 0}
        ...
        {phase === "rolling" && (
          <div className="max-w-2xl mx-auto">
            <div className="mb-10">
              ...
              <p className="text-amber-950/70 italic text-lg border-l-4 border-red-800/60 pl-4 font-medium">
                Confirme o resultado final de cada participante (d20 + bônus).
              </p>
            </div>
                ...
                return (
                  <div key={p.id} className="flex items-center gap-4 bg-[#e8dac1] border-2 border-amber-900/25 rounded-2xl p-4 shadow-sm">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {bonus !== null && (
                        <div className="text-xs text-amber-950/50">
                          Bônus: {bonus >= 0 ? "+" : ""}{bonus}
                        </div>
                      )}
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setRollValues((prev) => ({ ...prev, [p.id]: parseInt(e.target.value) || 0 }))
                        }
            ...
            <div className="flex gap-4 justify-between">
              <button
                onClick={() => {
                  setParticipants((prev) => prev.filter((p) => p.type === "player"));
                  setPhase("setup");
                }}
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-[#2a1810] text-[#fbf5e6] rounded-2xl px-6 py-4 shadow-xl">
              ...
              <div className="flex gap-3">
                <button
                  onClick={nextTurn}
                  className="px-6 py-3 bg-red-700 hover:bg-red-800 text-white font-black tracking-wider uppercase rounded-xl transition-all shadow-md text-sm"
                >
                  Encerrar Turno →
                </button>
                <button
                  onClick={endCombat}
                  className="px-5 py-3 border-2 border-[#fbf5e6]/20 text-[#fbf5e6]/60 hover:text-[#fbf5e6] hover:border-[#fbf5e6]/40 font-bold rounded-xl transition-all text-sm"
                >
                  Encerrar
                </button>
              </div>
            ...
            <div className="grid lg:grid-cols-3 gap-6 items-start">
              ...
              <div className="bg-[#2a1810] rounded-2xl p-4 shadow-xl sticky top-4">
                <h3 className="text-[10px] uppercase tracking-widest text-[#fbf5e6]/40 font-bold mb-3 border-b border-[#fbf5e6]/10 pb-2">
                  📜 Log de Combate
                </h3>
                <div className="space-y-1.5 max-h-[60vh] overflow-y-auto custom-scrollbar">
                  {log.map((entry, i) => (
                    <p
                      ...
                      className={`text-xs font-medium leading-snug ${
                        i === 0 ? "text-[#fbf5e6]/90" : "text-[#fbf5e6]/40"
                      }`}
                    >
                      {entry}
                    </p>
                  ...
                  {!log.length && (
                    <p className="text-xs text-[#fbf5e6]/20 italic">Nenhum evento ainda.</p>
                  )}
                </div>
              </div>
            </div>
      ...
      <footer className="relative z-10 mt-20 p-8 border-t-4 border-double border-amber-900/40 bg-[#2a1810] text-center font-serif flex flex-col items-center justify-center">
        ...
        <p className="mb-2 text-[#e8dac1]/60 text-sm tracking-widest uppercase font-bold">
          Compêndio Tormenta RPG © 2025
        </p>
        <p className="text-[#e8dac1]/40 text-xs tracking-wide">
